...
exports.dailyShiftMatching = functions
  .region("us-central1")
  .pubsub.schedule("0 2 * * *") // Run at 2 AM daily
  .timeZone("America/New_York")
  .onRun(async (context) => {
    console.log("Starting daily shift matching algorithm...");
    ...
    try {
      ...
      // Step 2: Check existing schedules for tomorrow to avoid conflicts
      const existingSchedulesSnapshot = await db
        .collection("schedules")
        .where("scheduled_date", ">=", admin.firestore.Timestamp.fromDate(tomorrow))
      ...
      for (const resident of availableResidents) {
        ...
        if (bestAttendee && availableTopics.length > 0) {
          ...
          const newSchedule = {
            ...
            // Scheduling data
            scheduled_date: admin.firestore.Timestamp.fromDate(tomorrow),
            ...
            // Metadata
            schedule_id: `${tomorrow.getFullYear()}${(tomorrow.getMonth() + 1).toString().padStart(2, "0")}${tomorrow.getDate().toString().padStart(2, "0")}_${resident.id}_${bestAttendee.id}`,
            test: false,
            schemaVersion: 2,
            migration_version: "1.0",
            created_by: "dailyShiftMatching",
      ...
      console.log("Daily shift matching completed successfully");
      return summary;
    } catch (error) {
      console.error("Error in daily shift matching:", error);
